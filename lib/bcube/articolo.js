 ...
 * Per le politiche MTS (scorta minima) integra dettagli scorta/lotto/lead time
 * se popolati nell'articolo.
 *
...
function buildPolitica(polCode, art) {
    ...
    // Solo per politiche a scorta (MTS) i dettagli scorta/lotto/lead time
    // sono parte significativa della descrizione operativa.
    if (row.pol === 'MTS') {
        const parts = [];
        if (art.ar_scomin)  parts.push(`scorta min. ${art.ar_scomin}`);
        if (art.ar_minord)  parts.push(`lotto ${art.ar_minord}`);
        if (art.ar_rrfence) parts.push(`lead time ${art.ar_rrfence} gg`);
