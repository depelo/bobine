...
const { getPoolUJET11, getPoolMRP, sql } = require('../config/db');
const { authenticateToken } = require('../middlewares/auth');
// TODO: configurare smtp-mrp.js per l'invio email ordini
...
async function caricaMRP(pool, codart, filtroMagaz, filtroFase) {
    ...
    // Leggi quantità emesse dalla nostra app (ordini_emessi su MRP)
    ...
    // Arricchisci ap_ordin con le quantità emesse dalla nostra app
...
// Funzione riusabile: deploy oggetti SQL sul pool MRP
...
// ============================================================
// FACTORY: crea il router con tutte le route MRP
// ============================================================
function createGb2Routes({ io }) {
    const router = express.Router();

    // ============================================================
    // API 1: RICERCA ARTICOLI (per le 3 combo della maschera parametri)
    // ============================================================
    router.get('/articoli/search', authenticateToken, async (req, res) => {
        try {
    ...
    // ============================================================
    router.get('/articoli/:codart/fasi', authenticateToken, async (req, res) => {
        try {
            const pool = await getPoolUJET11();
            const result = await pool.request()
    ...
    // ============================================================
    router.get('/magazzini', authenticateToken, async (req, res) => {
        try {
            const pool = await getPoolUJET11();
            const result = await pool.request()
                .query(`
    ...
    // ============================================================
    router.get('/progressivi', authenticateToken, async (req, res) => {
        try {
    ...
    // ============================================================
    router.get('/progressivi/expand', authenticateToken, async (req, res) => {
        try {
    ...
    // ============================================================
    router.get('/ordini-dettaglio', authenticateToken, async (req, res) => {
        try {
            const { codart, magaz, fase } = req.query;
            if (!codart) return res.status(400).json({ error: 'codart richiesto' });
    ...
    // ============================================================
    router.get('/ordini-rmp', authenticateToken, async (req, res) => {
        try {
    ...
    // ============================================================
    router.get('/ordini-padre', authenticateToken, async (req, res) => {
        try {
            const { codart, magaz, fase } = req.query;
            if (!codart) return res.status(400).json({ error: 'codart richiesto' });
    ...
    // ============================================================
    router.get('/consumi/sprint-multi', authenticateToken, async (req, res) => {
        try {
    ...
    // ============================================================
    router.get('/consumi/marathon-multi', authenticateToken, async (req, res) => {
        try {
            const codarts = String(req.query.codarts || '')
                .split(',')
                .map((c) => c.trim())
    ...
    // ============================================================
    router.get('/consumi/sprint/:codart', authenticateToken, async (req, res) => {
        try {
    ...
    // ============================================================
    router.get('/consumi/marathon/:codart', authenticateToken, async (req, res) => {
        try {
            const codart = req.params.codart;
            const pool = await getPoolUJET11();
    ...
    // ============================================================
    router.get('/proposta-ordini', authenticateToken, async (req, res) => {
        try {
            const pool = await getPoolUJET11();
    ...
    router.post('/deploy-sp', authenticateToken, async (req, res) => {
        try {
            const poolMRP = await getPoolMRP();
    ...
    router.get('/check-sp', authenticateToken, async (req, res) => {
        try {
            const poolMRP = await getPoolMRP();
    ...
    router.post('/emetti-ordine', authenticateToken, async (req, res) => {
        try {
            ...
            // Genera PDF
            // NOTE: In versione integrata non abbiamo getActiveProfile() per l'ambiente.
            // Si assume ambiente 'produzione' (nessun watermark).
    ...
    router.post('/emetti-ordini-batch', authenticateToken, async (req, res) => {
        try {
            ...
            for (const ord of ordini) {
                try {
                    ...
                    const ordine = result.recordsets[0][0];
                    const righeOrdine = result.recordsets[1] || [];
                    const pdfBuffer = await generaPdfOrdine(ordine, righeOrdine, { ambiente: 'produzione' });
    ...
    router.get('/ordine-pdf/:anno/:serie/:numord', authenticateToken, async (req, res) => {
        try {
    ...
    // ============================================================
    // API: CONFIGURAZIONE SMTP e INVIO EMAIL ORDINE FORNITORE
    // TODO: Queste route richiedono la configurazione di smtp-mrp.js.
    //       Riabilitarle dopo aver creato config/smtp-mrp.js.
    // ============================================================
    ...
    router.post('/invia-ordine-email', authenticateToken, async (req, res) => {
        try {
            ...
            // In versione integrata, assume sempre ambiente 'produzione'
            const ambiente = 'produzione';
            ...
            const oggetto = `NS/ ORDINE ${numord}_${serie} - ${ordine.fornitore_nome || ''}`;
